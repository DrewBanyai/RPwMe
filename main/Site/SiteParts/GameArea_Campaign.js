...
const { GameAreaCampaignJournal } = require('../SiteParts/GameArea_Campaign_Journal')
const { GameAreaCampaignMap } = require('../SiteParts/GameArea_Campaign_Map')
const { GameAreaCampaignInventory } = require('../SiteParts/GameArea_Campaign_Inventory')
const { GameAreaCampaignCombat } = require('../SiteParts/GameArea_Campaign_Combat')

const GameAreaCampaign = {
  create () {
    ...
    const screenTabs = [{ tab: 'Journal', command: '!journal' }, { tab: 'Map', command: '!map' }, { tab: 'Inventory', command: '!inventory' }, { tab: 'Combat', command: '!combat' }]
    ...
    container.elements.screens.Journal = GameAreaCampaignJournal.create({ style: { display: STYLE.GAME_WINDOW_MENU_DISPLAY_TYPE } })
    container.appendChild(container.elements.screens.Journal)

    container.elements.screens.Map = GameAreaCampaignMap.create({})
    container.appendChild(container.elements.screens.Map)

    container.elements.screens.Inventory = GameAreaCampaignInventory.create({})
    container.appendChild(container.elements.screens.Inventory)

    container.elements.screens.Combat = GameAreaCampaignCombat.create({})
    ...
    EventDispatch.AddEventHandler('Show Campaign Screen', (eventType, eventData) => {
      ...
      GameAreaCampaign.showSubscreen(container, eventData.screenID)
    })
  ...
  showSubscreen (container, screenID) {
    ...
    const keys = Object.keys(container.elements.screens)
    for (const k in keys) {
...
//  Module Exports
module.exports = { GameAreaCampaign }
