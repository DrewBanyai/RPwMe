...
const AdminAreaPlayers = {
  create () {
    ...
    AdminAreaPlayers.createStartPlayerJoinMenu(container)
    AdminAreaPlayers.createPlayerDataDisplay(container)

    AdminAreaPlayers.SetupEventHandlers(container)
  ...
  createStartPlayerJoinMenu (container) {
    ...
    const startPlayerJoinLabel1 = Label.create({
    ...
    const startPlayerJoinLabel2 = Label.create({
      id: 'StartPlayerJoinLabel',
      style: {
        fontFamily: 'Vesper Libre',
        fontSize: '36px',
    ...
    const startPlayerJoinButton = BasicButton.create({ id: 'StartPlayerJoinButton', style: STYLE.ADMIN_SCREEN_DEBUG_BUTTON, attributes: { value: 'Allow Players To Join' } })
  ...
  createPlayerDataDisplay (container) {
    ...
    AdminAreaPlayers.AddPlayerListUI(container)
  },
  ...
  AddPlayerListUI (container) {
    ...
    //  Active Player List
    const activePlayerListBox = Container.create({
    ...
    const activePlayerListTitle = Label.create({
    ...
    //  Player Request List
    const playerRequestListBox = Container.create({
    ...
    const joinRequestListTitle = Label.create({
    ...
    const beginGameOptionBox = Container.create({
    ...
    EventDispatch.AddEventHandler('!join', (eventType, eventData) => { AdminAreaPlayers.AddPlayerJoinRequest(container, eventData) })
    EventDispatch.AddEventHandler('!leave', (eventType, eventData) => { AdminAreaPlayers.RemovePlayer(container, eventData) })

    EventDispatch.AddEventHandler('!dwarf', (eventType, eventData) => { AdminAreaPlayers.SetPlayerRace(container, eventData) })
    EventDispatch.AddEventHandler('!elf', (eventType, eventData) => { AdminAreaPlayers.SetPlayerRace(container, eventData) })
    EventDispatch.AddEventHandler('!halfling', (eventType, eventData) => { AdminAreaPlayers.SetPlayerRace(container, eventData) })
    EventDispatch.AddEventHandler('!human', (eventType, eventData) => { AdminAreaPlayers.SetPlayerRace(container, eventData) })

    EventDispatch.AddEventHandler('!cleric', (eventType, eventData) => { AdminAreaPlayers.SetPlayerClass(container, eventData) })
    EventDispatch.AddEventHandler('!fighter', (eventType, eventData) => { AdminAreaPlayers.SetPlayerClass(container, eventData) })
    EventDispatch.AddEventHandler('!wizard', (eventType, eventData) => { AdminAreaPlayers.SetPlayerClass(container, eventData) })
    EventDispatch.AddEventHandler('!rogue', (eventType, eventData) => { AdminAreaPlayers.SetPlayerClass(container, eventData) })

    EventDispatch.AddEventHandler('!name', (eventType, eventData) => { AdminAreaPlayers.SetPlayerName(container, eventData) })

    EventDispatch.AddEventHandler('!reroll', (eventType, eventData) => { AdminAreaPlayers.RerollCharacter(container, eventData) })
    EventDispatch.AddEventHandler('!ready', (eventType, eventData) => { AdminAreaPlayers.ReadyCharacter(container, eventData) })
  },
  ...
  SetupEventHandlers (container) {
    ...
    EventDispatch.AddEventHandler('Start Player Join Mode', (eventType, eventData) => {
      ...
      AdminAreaPlayers.UpdatePlayerData(container)
    ...
    EventDispatch.AddEventHandler('Player Ready Status Updated', (eventType, eventData) => {
      let playersReady = 0
      const playersList = CampaignController.GetPlayersList()
      const playersListKeys = Object.keys(playersList)
  ...
  AddActivePlayerEntry (container, eventData) {
    const player = ActivePlayerEntry.create({
      name: eventData.user,
      selectCallback: () => {
  ...
  AddPlayerJoinRequest (container, eventData) {
    ...
    const player = PlayerJoinRequest.create({ name: eventData.user })
    PlayerJoinRequest.setApproveCallback(player, () => { AdminAreaPlayers.ApprovePlayerJoinRequest(container, eventData, player) })
    PlayerJoinRequest.setDenyCallback(player, () => { AdminAreaPlayers.DenyPlayerJoinRequest(container, eventData, player) })
    container.elements.playerRequestList.appendChild(player)
    container.requestUsers.push(eventData.user)

    if (SETTINGS.ADMIN_SETTINGS.AutoApproveJoinRequests) { AdminAreaPlayers.ApprovePlayerJoinRequest(container, eventData, player) }
  },
  ...
  RemovePlayer (container, eventData) {
    if (container.requestUsers.includes(eventData.user)) {
      ...
      for (let i = 0; i < container.elements.playerRequestList.children.length; ++i) {
        if (container.elements.playerRequestList.children[i].username === eventData.user) { playerEntry = container.elements.playerRequestList.children[i] }
      }
    ...
    if (CampaignController.GetPlayerExists(eventData.user)) {
      ...
      for (let i = 0; i < container.elements.activePlayerList.children.length; ++i) {
        if (container.elements.activePlayerList.children[i].username === eventData.user) { playerEntry = container.elements.activePlayerList.children[i] }
      }
  ...
  SetPlayerRace (container, eventData) {
    ...
    const player = CampaignController.GetPlayer(eventData.user)
  ...
  ApprovePlayerJoinRequest (container, eventData, joinRequest) {
    ...
    AdminAreaPlayers.AddActivePlayerEntry(container, eventData)
    AdminAreaPlayers.UpdatePlayerData(container)
  },
  ...
  UpdatePlayerData (container) {
    ...
    // const playerDataDisplay = container.elements.playerDataDisplay
  }
}

//  Module Exports
module.exports = { AdminAreaPlayers }
