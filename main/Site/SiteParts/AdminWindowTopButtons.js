...
const AdminWindowTopButtons = {
  ...
  createAdminWindowTopButton: (topButtons, buttonData) => {
    ...
    const buttonContainer = Container.create({
      id: 'ButtonContainer',
      style: {
    ...
    const button = Fontawesome.create({
    ...
    button.onmouseover = () => {
      const selected = (topButtons.selectedButton === button)
  ...
  createAdminWindowTopDivider: (buttonStrip) => {
    const divider = Container.create({
      id: 'TopDivider',
      style: {
        width: '1px',
        height: pxFromInt(STYLE.ADMIN_WINDOW_BUTTON_HEIGHT),
