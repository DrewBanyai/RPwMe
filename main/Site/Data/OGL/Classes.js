...
const { ABILITY_SCORES } = require('./AbilityScores')
const { CHARACTER } = require('./Character')
const { EQUIPMENT } = require('./Equipment')

const { DiceRoller } = require('../../HelperFunctions/DiceRoller')
...
const CLASSES = {
  ...
  Cleric: {
    ...
    AssignClassTraits (character) {
      ...
      const clericWeaponProfs = EQUIPMENT.GetEquipmentNameList(['Simple Melee', 'Simple Ranged'], [])
      ...
      const clericArmorProfs = EQUIPMENT.GetEquipmentNameList([], ['Light Armor', 'Heavy Armor', 'Shield'])
      ...
      const clericSkills = CLASSES.Cleric.GetStartingSkillProficiencies()
    ...
    GetClassStartingEquipment (character) {
      ...
      const weapon = ['Mace', 'Warhammer'][RandIntBetween(0, character.WeaponProficiencies.Warhammer ? 1 : 0)]
      equipment.push({ item: weapon, count: 1 })

      const armor = ['Scale Mail Armor', 'Leather Armor', 'Chain Mail Armor'][RandIntBetween(0, character.ArmorProficiencies['Chain Mail Armor'] ? 2 : 1)]
      equipment.push({ item: armor, count: 1 })

      const extraWeapon = ['Crossbow, light', EQUIPMENT.WEAPONS['Simple Melee'][RandIntBetween(0, EQUIPMENT.WEAPONS['Simple Melee'].length - 1)].name][RandIntBetween(0, 1)]
      equipment.push({ item: extraWeapon, count: 1 })
      if (extraWeapon === 'Crossbow, light') equipment.push({ item: 'Bolt', count: 20 })

      const clericPack = ["Priest's Pack", "Explorer's Pack"][RandIntBetween(0, 1)]
    ...
    DetermineWeaponOfChoice (character) {
      const allWeapons = EQUIPMENT.GetAllWeaponsInOneList()
      const inventory = character.Inventory

      const ownedWeapons = []
      for (const itemIndex in inventory) {
        const equipmentItem = allWeapons.find((w) => w.name === inventory[itemIndex].item)
        if (equipmentItem) { ownedWeapons.push(equipmentItem) }
      }
      if (ownedWeapons.length === 0) { return 'None' }

      //  Cleric Preference 1: Mace
      const mace = ownedWeapons.find((w) => w.name.toLowerCase().includes('mace'))
      if (mace) {
        const shield = !mace.properties.includes('two-handed') && inventory.find((i) => i.item === 'Shield')
        if (shield) {
          //  TODO: Equip items
      ...
      const hammer = ownedWeapons.find((w) => w.name.toLowerCase().includes('hammer'))
      if (hammer) {
        const shield = !hammer.properties.includes('two-handed') && inventory.find((i) => i.item === 'Shield')
        if (shield) {
          //  TODO: Equip items
      ...
      const bow = ownedWeapons.find((w) => w.name.toLowerCase().includes('bow'))
      if (bow) {
        //  TODO: Equip items
        return bow.name
      }
  ...
  Fighter: {
    ...
    AssignClassTraits (character) {
      ...
      const fighterWeaponProfs = EQUIPMENT.GetEquipmentNameList(['Simple Melee', 'Simple Ranged', 'Martial Melee', 'Martial Ranged'], [])
      if (!character.WeaponProficiencies) character.WeaponProficiencies = {}
      Object.assign(character.WeaponProficiencies, CHARACTER.CreateWeaponProficiencyData(fighterWeaponProfs))

      const fighterArmorProfs = EQUIPMENT.GetEquipmentNameList([], ['Light Armor', 'Medium Armor', 'Heavy Armor', 'Shield'])
      if (!character.ArmorProficiencies) character.ArmorProficiencies = {}
      ...
      const fighterSkills = CLASSES.Fighter.GetStartingSkillProficiencies()
      if (!character.SkillProficiencies) character.SkillProficiencies = {}
      ...
      const fightingStyleNote = fightingStyles[RandIntBetween(0, fightingStyles.length - 1)]
      character.Notes = [
        fightingStyleNote,
    ...
    GetClassStartingEquipment (character) {
      ...
      const martialWeapons = EQUIPMENT.GetEquipmentNameList(['Martial Melee'], [])
      if (RandIntBetween(0, 1) === 0) {
        equipment.push({ item: martialWeapons[RandIntBetween(0, martialWeapons.length - 1)], count: 1 })
        equipment.push({ item: 'Shield', count: 1 })
      } else {
        const martialWeapon1 = martialWeapons[RandIntBetween(0, martialWeapons.length - 1)]
    ...
    DetermineWeaponOfChoice (character) {
      ...
      const martialM = ownedWeapons.find((w) => ['Martial Melee', 'Martial Ranged'].includes(w.subtype))
      if (martialM) {
        const shield = !martialM.properties.includes('two-handed') && inventory.find((i) => i.item === 'Shield')
        if (shield) {
          //  TODO: Equip items
          return martialM.name + ' and Shield'
        } else {
          const inventoryEntry = inventory.filter((i) => i.item === martialM.name)
          //  TODO: Equip items
      ...
      const martialR = ownedWeapons.find((w) => ['Martial Ranged'].includes(w.subtype))
      if (martialR) {
        const shield = !martialR.properties.includes('two-handed') && inventory.find((i) => i.item === 'Shield')
        if (shield) {
          //  TODO: Equip items
          return martialR.name + ' and Shield'
        } else {
          const inventoryEntry = inventory.filter((i) => i.item === martialR.name)
          //  TODO: Equip items
  ...
  Rogue: {
    ...
    AssignClassTraits (character) {
      ...
      const rogueWeaponProfs = EQUIPMENT.GetEquipmentNameList(['Simple Melee', 'Simple Ranged'], []).concat(['Crossbow, hand', 'Longsword', 'Rapier', 'Shortsword'])
      if (!character.WeaponProficiencies) character.WeaponProficiencies = {}
      Object.assign(character.WeaponProficiencies, CHARACTER.CreateWeaponProficiencyData(rogueWeaponProfs))

      const rogueArmorProfs = EQUIPMENT.GetEquipmentNameList([], ['Light Armor'])
      if (!character.ArmorProficiencies) character.ArmorProficiencies = {}
      ...
      const rogueSkills = CLASSES.Rogue.GetStartingSkillProficiencies()
      if (!character.SkillProficiencies) character.SkillProficiencies = {}
    ...
    GetClassStartingEquipment (character) {
      ...
      const pack = ChooseXFromList(1, ["Burglar's Pack", "Dungeoneer's Pack", "Explorer's Pack"])[0]
    ...
    DetermineWeaponOfChoice (character) {
      ...
      const sword = ownedWeapons.find((w) => ['Rapier', 'Shortsword'].includes(w.name))
      if (sword) {
        const shield = !sword.properties.includes('two-handed') && inventory.find((i) => i.item === 'Shield')
        if (shield) {
          //  TODO: Equip items
  ...
  Wizard: {
    ...
    AssignClassTraits (character) {
      ...
      const wizardWeaponProfs = CHARACTER.CreateWeaponProficiencyData(['Dagger', 'Dart', 'Sling', 'Quarterstaff', 'Crossbow, light'])
      if (!character.WeaponProficiencies) character.WeaponProficiencies = {}
      ...
      const wizardSkills = CLASSES.Wizard.GetStartingSkillProficiencies()
      if (!character.SkillProficiencies) character.SkillProficiencies = {}
    ...
    DetermineWeaponOfChoice (character) {
      ...
      const staff = ownedWeapons.find((w) => w.name.toLowerCase().includes('staff'))
      if (staff) {
        //  TODO: Equip items
        return staff.name
      }
      ...
      const dagger = ownedWeapons.find((w) => w.name.toLowerCase().includes('dagger'))
      if (dagger) {
        //  TODO: Equip items
        return dagger.name
      }
