...
const EQUIPMENT = {
    ...
    GetAllWeaponsInOneList() {
        let list = [];
        for (let entryIndex1 in this.WEAPONS) list = list.concat(this.WEAPONS[entryIndex1]);
        return list;
    },
    GetAllArmorInOneList() {
        let list = [];
        for (let entryIndex1 in this.ARMOR) list = list.concat(this.ARMOR[entryIndex1]);
        return list;
    },
    GetCharacterArmorClass(character) {
        let armorNames = this.GetEquipmentNameList([], ["Light Armor", "Medium Armor", "Heavy Armor", "Shield"]);
        let armorOwned = character.Inventory.filter((item) => armorNames.includes(item.item));
        let allArmor = this.GetAllArmorInOneList();

        //  If the character has armor, determine their AC based on it
        let armorClass = 0;
        for (let itemIndex in armorOwned)
            armorClass += (allArmor.find(armor => armor.name == armorOwned[itemIndex].item)).armorClass(character.AbilityScoreModifiers);
