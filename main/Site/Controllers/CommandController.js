...
const CommandControl = {
  ...
  GetCommandData: (commandName) => {
    ...
    const commandKeys = Object.keys(CommandControl.COMMANDS_LIST)
  ...
  CheckIsCommandValid: (commandString, userType) => {
    ...
    const commandParsed = CommandControl.GetCommandStringParsed(commandString)
    const commandData = CommandControl.GetCommandData(commandParsed.id)
  ...
  CommandRequest: (username, commandString, userType) => {
    const commandValid = CommandControl.CheckIsCommandValid(commandString, userType)
    ...
    const eventData = { user: username, command: commandParsed.id, data: commandData, args: commandParsed.args }
  ...
  GetCommandEffect: (commandQuery) => {
    ...
    const commandParts = commandQuery.split(' ')
    if (commandParts > 1) { return null }

    const commandID = '!' + commandParts[0].substring(1, commandParts[0].length - 1)
    if (commandID.length <= 0) { return null }

    const commandData = CommandControl.GetCommandData(commandID)
    if (!commandData) { return null }

    return commandData.effect
  },
  InitCommandControl: () => {
    const commandIDs = Object.keys(CommandControl.COMMANDS_LIST)
    commandIDs.forEach(commandID => {
      TwitchControl.AddCommandCallback(commandID, (userstate, message) => {
        const userIsGM = (userstate.username === CampaignController.GetCampaignGameMaster())
        const userIsPlayer = (CampaignController.GetPlayerExists(userstate.username))
        const userType = (userIsGM ? 'gm' : (userIsPlayer ? 'player' : ('viewer')))
